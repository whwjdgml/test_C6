...
SensorManager::SensorManager() : aht20_sensor(nullptr), bmp280_sensor(nullptr), scd41_sensor(nullptr), 
                                 battery_ntc_sensor(nullptr), battery_heater(nullptr),
                                 aht20_initialized(false), bmp280_initialized(false), scd41_initialized(false), sgp40_initialized(false), ina226_initialized(false), rtc_initialized(false), battery_heater_initialized(false),
...
bool SensorManager::testPullupResistors() {
    ...
    // --- 배터리 모니터링 및 히터 제어 설정 ---
    #define BATT_ADC_PIN                ADC_CHANNEL_0 // 서미스터가 연결된 ADC 채널 (예: GPIO0)
    #define HEATER_PWM_PIN              GPIO_NUM_1    // 히터 제어용 PWM 핀 (예: GPIO1)
    
    // 서미스터 설정 (NTC 100k, B=3950)
    #define THERMISTOR_NOMINAL_R        100000.0f // 25°C에서의 저항 (100kΩ)
    #define THERMISTOR_B_VALUE          3950.0f   // B-상수
    #define THERMISTOR_NOMINAL_T_C      25.0f     // 공칭 온도 (섭씨)
    #define VOLTAGE_DIVIDER_R           100000.0f // 서미스터와 연결된 고정 저항 (100kΩ)
    #define ADC_V_IN                    3.3f      // ADC 회로에 공급되는 전압
    
    // 히터 제어 로직 설정
    #define HEATER_TARGET_TEMP          10.0f     // 목표 유지 온도 (이 온도 이상이면 히터 OFF)
    #define HEATER_ON_TEMP              5.0f      // 히터가 100%로 켜지는 하한 온도
    ...
    #define HEATER_LEDC_RESOLUTION      LEDC_TIMER_10_BIT // 10-bit 해상도 (0-1023)
    #define HEATER_LEDC_FREQ_HZ         1000              // PWM 주파수 (1kHz)
    typedef struct {
        float voltage;
        float current;
        float power;
        ...
        bool  is_heater_active;    // 히터 동작 여부
        int   heater_duty_cycle;   // 히터 PWM 듀티 사이클 (%)
    } BatteryStatus;
...
// 배터리 히터 시스템 기능 구현
bool SensorManager::initBatteryHeater() {
    if (!battery_ntc_sensor) {
        battery_ntc_sensor = std::make_unique<NTCSensor>(BATTERY_NTC_ADC_CHANNEL, BATTERY_NTC_POWER_PIN);
        if (!battery_ntc_sensor->init()) {
            ESP_LOGE(TAG, "배터리 NTC 센서 초기화 실패");
            return false;
        }
    }
    
    if (!battery_heater) {
        battery_heater = std::make_unique<BatteryHeater>(battery_ntc_sensor.get(), STEPUP_CONVERTER_EN_PIN, BATTERY_HEATER_PWM_PIN);
        if (!battery_heater->init()) {
            ESP_LOGE(TAG, "배터리 히터 초기화 실패");
            return false;
        }
    }
...
bool SensorManager::updateBatteryHeater() {
    if (!battery_heater) {
        ESP_LOGW(TAG, "배터리 히터가 초기화되지 않음");
        return false;
    }
    
    return battery_heater->updateHeater();
}

float SensorManager::getBatteryTemperature() {
    if (!battery_ntc_sensor) {
        ESP_LOGW(TAG, "배터리 NTC 센서가 초기화되지 않음");
        return -999.0f;  // 오류 값
    }
    
    float temperature = -999.0f;
    if (battery_ntc_sensor->readTemperature(&temperature)) {
        return temperature;
    } else {
...
heater_state_t SensorManager::getHeaterState() {
    if (!battery_heater) {
        return HEATER_UNKNOWN;
    }
    
    return battery_heater->getCurrentState();
}

void SensorManager::emergencyHeaterShutdown() {
    if (battery_heater) {
        battery_heater->emergencyShutdown();
        ESP_LOGI(TAG, "⚡ 배터리 히터 비상 정지");
    }
}

void SensorManager::printHeaterStatus() {
    if (!battery_heater) {
        ESP_LOGI(TAG, "배터리 히터: 초기화되지 않음");
        return;
    }
    
    battery_heater->printStatus();
}

void SensorManager::printHeaterStatistics() {
    if (!battery_heater) {
        ESP_LOGI(TAG, "배터리 히터: 통계 없음");
        return;
    }
    
    battery_heater->printStatistics();
}